...
<xsl:param name="fake.filename" />
<xsl:param name="fake.version" />
<xsl:param name="fake.project.name" />
<xsl:param name="fake.project.buildfile" />
<xsl:param name="fake.project.basedir" />
<xsl:param name="fake.project.default" />
...
<xsl:template name="header">
    <xsl:param name="path"/>
    <h1><span id=":i18n:UnitTestsResults">Unit Tests Results</span> - <xsl:value-of select="$fake.project.name"/></h1>
    <table width="100%">
    <tr>
       <td align="left">
          <span id=":i18n:GeneratedBy">Auto generated by FAKE build script </span> : <xsl:value-of select="@date"/> - <xsl:value-of select="concat(@time,' ')"/>
       </td>
    </tr>
    </table>
    <hr size="1"/>
...
<xsl:template name="summaryHeader">
    <tr valign="top" class="TableHeader">
        ...
        <td width="70px"><b id=":i18n:Ignored">Ignored</b></td>
...
<xsl:template name="packageSummaryHeader">
    <tr class="TableHeader" valign="top">
        ...
        <td width="5%"><b id=":i18n:Ignored">Ignored</b></td>
...
-->
    <xsl:template name="summary">
        ...
        <xsl:variable name="total" select="@total + @ignored"/>
        <xsl:variable name="successCount" select="@total - @failures - @errors"/>
        <xsl:variable name="failureCount" select="@failures"/>
        <xsl:variable name="errorCount" select="@errors"/>
        <xsl:variable name="allFailureCount" select="@failures + @errors"/>
        ...
        <tr valign="top">
            ...
            <td>
                <xsl:if test="round($total * 200 div $total )!=0">
                    <span class="covered">
                        <xsl:attribute name="style">
                            width:<xsl:value-of select="round($total * 200 div $total )"/>px
                        </xsl:attribute>
                    </span>
                </xsl:if>
                ...
                <xsl:if test="round($allFailureCount * 200 div $total )!=0">
                    ...
                    <xsl:if test="round($ignoreCount * 200 div $total )!=0">
                        <span class="uncovered">
                            <xsl:attribute name="style">
                                width:<xsl:value-of select="round($allFailureCount * 200 div $total )"/>px;
                                margin-left:<xsl:value-of select="round($ignoreCount * 200 div $total )"/>px;
                            </xsl:attribute>
                        </span>
                    </xsl:if>
                </xsl:if>
...
<xsl:template match="test-case">
   ...
   <xsl:variable name="result">
            <xsl:choose>
                <xsl:when test="@result='Failure'"><span id=":i18n:Failure">Failure</span></xsl:when>
                <xsl:when test="@result='Error'"><span id=":i18n:Error">Error</span></xsl:when>
                <xsl:when test="@result='Ignored'"><span id=":i18n:Ignored">Ignored</span></xsl:when>
